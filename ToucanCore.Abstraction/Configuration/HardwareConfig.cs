...
namespace ToucanCore.Abstraction.Configuration
{
    public class HardwareConfig :  ICloneable
    {
        public const string DefaultFileName = ".default.hws";
        public const string XMLTag = "Hardware";

        public static List<IStartTrigger> StartTriggers { get; } = new List<IStartTrigger>();
        ...
        public static List<IFixture> Fixtures { get; private set; } = new List<IFixture>();
        public static List<ISerialNumberReader> SerialNumberReaders { get; private set; } = new List<ISerialNumberReader>();
        public static List<IRelayArray> RelayArrays { get; private set; } = new List<IRelayArray>();
                    ...
                    if (t.IsVisible && !t.IsAbstract && !t.IsInterface)
                    {
                        ...
                        foreach (var intf in intfs)
                        {
                            if (intf == typeof(IFixture))
                            {
                                ...
                                {
                                    RelayArrays.Add((IRelayArray)obj);
                                    continue;
                                }
                            ...
                            {
                                if (!RelayArrays.Exists(x => x.GetType() == t))
                                {
                                    RelayArrays.Add(Activator.CreateInstance(t) as IRelayArray);
                                }
                            }
                            else if (intf == typeof(ISerialNumberReader))
                            {
                                if (!SerialNumberReaders.Exists(x => x.GetType() == t))
                                {
                                    SerialNumberReaders.Add(Activator.CreateInstance(t) as ISerialNumberReader);
                                }
                            }
                        }
                    }
        ...
        public HardwareConfig(XElement element) : this()
        {
            ...
            if (e_fixture != null)
            {
                ...
                if (Fixture != null)
                {
                    ...
                    Fixture.AutoDutIn = HalHelper.TRUE_STRING.Contains(e_fixture.Attribute("autodutin")?.Value);
                    Fixture.AutoDutOut = HalHelper.TRUE_STRING.Contains(e_fixture.Attribute("autodutout")?.Value);
                }
            }
            ...
            if (e_snreader != null)
            {
                ...
                if (SerialNumberReader != null)
                {
                    ...
                    if (HalHelper.TRUE_STRING.Contains(e_snreader.Attribute("trigondutpresent")?.Value))
                    {
