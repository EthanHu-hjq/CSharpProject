...
namespace Mes
{
    public sealed class TYMSFC : TF_Base, IInternalMes
    {
        public static SPD.ISQLServer Server { get; private set; }
        public const string DEFAULT_GET_PART_API = "Get_model";
        public static string Version = "1.0";
        public static int ConnectionTimeOut_ms = 5000;

        const string SFCsHeader = "BARCODE_PART,BARCODE,Model_NO,Line_Code,Station_Code,PROCESS_NO,Result,Defect_Code,Defect_Info,update_user,GuiVer,errmsg,report";
        ...
        {
            Debug(string.Format("GetDate params:{0}", sfcconfig.Product));

            try
            {
                var rtn = Server.GetDate(sfcconfig.Product);
                Debug($"GetDate rtn: {rtn}");

                var dt = DateTime.Parse(rtn);
        ...
        {
            Debug($"GetModel params:{sfcconfig.Product}|{sfcconfig.GetPartNoApi}|{sn}");
            try
            {
                string rtn = null;
                lock (checkingSfcsLock)
                {
                    rtn = Server.GetSpecialValue(sfcconfig.Product, sfcconfig.GetPartNoApi ?? DEFAULT_GET_PART_API, sn);
                }
                ...
                {
                    throw new MesException($"{GetType().Name} GetModel Failed(获取机型码失败). SFCs Return :{rtn}");
                }
                else if (componenet[0].Length < 1)
                {
                    throw new MesException($"{GetType().Name} GetModel Failed(获取机型码失败). PartNo Is Empty. SFCs Return :{rtn}");
                }
                else if (componenet[1].Length < 1)
                {
                    throw new MesException($"{GetType().Name} GetModel Failed(获取机型码失败). LineNo Is Empty. SFCs Return :{rtn}");
                }
                else
                {
        ...
        {
            Debug($"CheckStation params:{sfcconfig.Product}|{sfcconfig.Station}|{sn}|{lineno}|{sfcconfig.Version}");
            try
            {
                string rtn = null;
                lock (checkingSfcsLock)
                {
                    rtn = Server.CheckStationPass(sfcconfig.Product, sfcconfig.Station, sn, lineno, sfcconfig.Version);
                }
        ...
        public string ExecMesApi(IMesConfig sfcconfig, string api, params string[] parameters)
        {
            ...
            var rtn = Server.GetSpecialValue(sfcconfig.Product, api, para);
        ...
        public void CommitMesResult(ITestResult rs, string specialdata, string extcol, string extval)
        {
            ...
            if (tymrs.SFCsConfig.EnableSfc && tymrs.IsSFC && tymrs.SerialNumber.Length > 2)
            {
                ...
                switch (tymrs.Status)
                {
                    case TF_TestStatus.PASSED: sts = "PASS"; break;
                    case TF_TestStatus.ERROR: 
                        sts = "ERROR";
                        errmsg = tymrs.ErrorMessage.Info.Replace(",", " ");
                ...
                var sfcsstaticdata = $"{specialdata},{tymrs.SerialNumber},{tymrs.PartNo},{tymrs.LineNo},{tymrs.SFCsConfig.Station},{tymrs.SFCsConfig.Station},{sts},{tymrs.GetDefectCodes(";", 1)},{tymrs.GetDefectDescs(";", 1)},{Operator},{GuiVersion},{errmsg},{tymrs.GenerateReportName(null)}";
                ...
                if (!IsForValidation)
                {
                    lock (commitingSfcsLock)
                    {
                        ...
                        if (rtn != "Y")
                        {
                            throw new MesException($"{GetType().Name} InsertIntoTable Failed(插入数据失败). Product: {tymrs.SFCsConfig.Product}, Table: {tymrs.SFCsConfig.SfcsTable}\r\n Col: {sfccol}\r\nVal: {sfcdata}\r\nSFCs Return :{rtn}");
                        }
                    }
                }
            }
